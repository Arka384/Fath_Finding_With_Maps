  ...
  for(let i=0;i<grid.length;i++){
    for(let j=0;j<grid[i].length;j++){
      if(grid[i][j].isWall === true)
        ...
        fill(255, 0, 0);
      else if(grid[i][j].isSource === true)
        fill(0, 255, 0);
      else if(grid[i][j].isDest === true)
        fill(0, 0, 255);
      else
        fill(160, 160, 160);
...
class cell {
  constructor(isWall, isVisited, vertexNumber, x, y) {
    ...
    this.isSource = false;  //if soruce or dest
    this.isDest = false;
...
function init() {
  ...
  for(let i=0;i<n;i++){
    ...
    for(let j=0;j<n;j++){
      ...
      if(colour === colorBlack)
        tempOb.toggel();  //basically make it wall
...
function mousePressed() {
  //update maze with walls
  for(let i=0;i<grid.length;i++){
    for(let j=0;j<grid[i].length;j++){
      let posX = grid[i][j].y;
      let posY = grid[i][j].x;
      let size =  grid[i][j].size;
      if(mouseX > posX && mouseX < posX + 20 && mouseY > posY && mouseY < posY + 20){
        grid[i][j].toggel();
      }
    }
  }
...
function keyPressed() {
  ...
  if(keyCode ===  13) { //if "Enter" is pressed then create data structrue and run bfs on it.
    adj = new Array();
